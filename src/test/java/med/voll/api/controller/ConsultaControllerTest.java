...
class ConsultaControllerTest {
    ...
    void agendar_Cenario2() throws Exception {
        ...
        var response = mockMvc.perform(post("/consultas").contentType(MediaType.APPLICATION_JSON).content(dadosAgendamentoConsultaJacksonTester.write(
                new DadosAgendamentoConsulta(2l, 5l, data, especialidade)).getJson())).andReturn().getResponse();

        assertThat(response.getStatus()).isEqualTo(HttpStatus.OK.value());

        var jsonEsperado = dadosDetalhamentoConsultaJacksonTester.write(new DadosDetalhamentoConsulta(null, 2l, 5l, data)).getJson();
